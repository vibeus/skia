...
static constexpr int kNumGrPrimitiveTypes =
    (int)GrPrimitiveType::kLineStrip + 1;
...
// A DDL recorder has its own proxy provider and proxy cache. This enum
// indicates if a given proxy provider is one of these special ones.
...
enum class GrWrapCacheable : bool {
  /**
   * The wrapped resource will be removed from the cache as soon as it becomes
   * purgeable. It may still be assigned and found by a unique key, but the
   * presence of the key will not be used to keep the resource alive when it has
   * no references.
   */
  kNo = false,
  /**
   * The wrapped resource is allowed to remain in the GrResourceCache when it
   * has no references but has a unique key. Such resources should only be given
   * unique keys when it is known that the key will eventually be removed from
   * the resource or invalidated via the message bus.
   */
  kYes = true
};
...
enum class GrBudgetedType : uint8_t {
  ...
  kBudgeted,
  /**
   * The resource is unbudgeted and is purged as soon as it has no refs
   * regardless of whether it has a unique or scratch key.
   */
  kUnbudgetedUncacheable,
  /**
   * The resource is unbudgeted and is allowed to remain in the cache with no
   * refs if it has a unique key. Scratch keys are ignored.
   */
...
struct GrMipLevel {
  ...
  size_t fRowBytes = 0;
  // This may be used to keep fPixels from being freed while a GrMipLevel
  // exists.
 ...
 * OpsTask/GrOpsRenderPass begins execution.
 */
enum class GrLoadOp {
  kLoad,
  kClear,
 ...
 * OpsTask/GrOpsRenderPass ends execution.
 */
...
/**
 * Some pixel configs are inherently clamped to [0,1], some are allowed to go
 * outside that range, and some are FP but manually clamped in the XP.
 */
...
/**
 * A number of rectangle/quadrilateral drawing APIs can control anti-aliasing on
 * a per edge basis. These masks specify which edges are AA'ed. The intent for
 * this is to support tiling with seamless boundaries, where the inner edges are
 * non-AA and the outer edges are AA. Regular rectangle draws simply use kAll or
 * kNone depending on if they want anti-aliasing or not.
 *
 * In APIs that support per-edge AA, GrQuadAAFlags is the only AA-control
 * parameter that is provided (compared to the typical GrAA parameter). kNone is
 * equivalent to GrAA::kNo, and any other set of edge flags would require
 * GrAA::kYes (with rendering output dependent on how that maps to GrAAType for
 * a given SurfaceDrawContext).
 *
...
/** Rectangle and external textures only support the clamp wrap mode and do not
 * support MIP maps.
 */
...
enum GrVertexAttribType {
  ...
  kUByte_norm_GrVertexAttribType,   // unsigned byte, e.g. coverage, 0 -> 0.0f,
                                    // 255 -> 1.0f.
  kUByte4_norm_GrVertexAttribType,  // vector of 4 unsigned bytes, e.g. colors,
                                    // 0 -> 0.0f, 255 -> 1.0f.
  ...
  kUShort4_norm_GrVertexAttribType,  // vector of 4 unsigned shorts. 0 -> 0.0f,
                                     // 65535 -> 1.0f.
 ...
 * factories that take a GrClipEdgeType will succeed with all values and it is
 * up to the caller to verify success.
 */
...
static const int kGrGpuBufferTypeCount =
    static_cast<int>(GrGpuBufferType::kUniform) + 1;
...
enum class GrInternalSurfaceFlags {
  ...
  // Means the pixels in the texture are read-only. Cannot also be a
  // GrRenderTarget[Proxy].
  kReadOnly = 1 << 0,
  ...
  // This means the render target is multisampled, and internally holds a
  // non-msaa texture for resolving into. The render target resolves itself by
  // blitting into this internal texture. (asTexture() might or might not return
  // the internal texture, but if it does, we always resolve the render target
  // before accessing this texture's data.)
  kRequiresManualMSAAResolve = 1 << 2,

  // This means the pixels in the render target are write-only. This is used for
  // Dawn and Metal swap chain targets which can be rendered to, but not read or
  // copied.
  kFramebufferOnly = 1 << 3,

  // This is a Vulkan only flag. If set the surface can be used as an input
  // attachment in a shader. This is used for doing in shader blending where we
  // want to sample from the same image we are drawing to.
  kVkRTSupportsInputAttachment = 1 << 4,
};

GR_MAKE_BITFIELD_CLASS_OPS(GrInternalSurfaceFlags)

// 'GR_MAKE_BITFIELD_CLASS_OPS' defines the & operator on GrInternalSurfaceFlags
// to return bool. We want to find the bitwise & with these masks, so we declare
// them as ints.
constexpr static int kGrInternalTextureFlagsMask =
    static_cast<int>(GrInternalSurfaceFlags::kReadOnly);

// We don't include kVkRTSupportsInputAttachment in this mask since we check it
// manually. We don't require that both the surface and proxy have matching
// values for this flag. Instead we require if the proxy has it set then the
// surface must also have it set. All other flags listed here must match on the
// proxy and surface.
// TODO: Add back kFramebufferOnly flag here once we update
// SkSurfaceCharacterization to take it as a flag. skbug.com/10672
 ...
 * many-to-many. It does not specify the sRGB encoding of the stored values. The
 * components are listed in order of where they appear in memory. In other words
 * the first component listed is in the low bits and the last component in the
 * high bits.
 */
enum class GrColorType {
  ...
  // Unusual types that come up after reading back in cases where we are
  // reassigning the meaning of a texture format's channels to use for a
  // particular color format but have to read back the data to a full RGBA
  // quadruple. (e.g. using a R8 texture format as A8 color type but the API
  // only supports reading to RGBA8.) None of these have SkColorType
  // equivalents.
  kAlpha_8xxx,
  kAlpha_F32xxx,
  kGray_8xxx,
...
/**
 * Describes a GrColorType by how many bits are used for each color component
 * and how they are encoded. Currently all the non-zero channels share a single
 * GrColorTypeEncoding. This could be expanded to store separate encodings and
 * to indicate which bits belong to which components.
 */
class GrColorFormatDesc {
 public:
  ...
  static constexpr GrColorFormatDesc MakeRGBA(int rgb, int a,
  ...
  static constexpr GrColorFormatDesc MakeRGB(int r, int g, int b,
                                             GrColorTypeEncoding e) {
    return {r, g, b, 0, 0, e};
  }
  ...
  static constexpr GrColorFormatDesc MakeGray(int grayBits,
                                              GrColorTypeEncoding e) {
    return {0, 0, 0, 0, grayBits, e};
  }

  static constexpr GrColorFormatDesc MakeGrayAlpha(int grayAlpha,
                                                   GrColorTypeEncoding e) {
 ...
 private:
  ...
  constexpr GrColorFormatDesc(int r, int g, int b, int a, int gray,
                              GrColorTypeEncoding encoding)
      : fRBits(r),
        fGBits(g),
        fBBits(b),
        fABits(a),
...
static constexpr GrClampType GrColorTypeClampType(GrColorType colorType) {
  if (GrGetColorTypeDesc(colorType).encoding() == GrColorTypeEncoding::kUnorm ||
      GrGetColorTypeDesc(colorType).encoding() ==
          GrColorTypeEncoding::kSRGBUnorm) {
    return GrClampType::kAuto;
  }
  return GrColorType::kRGBA_F16_Clamped == colorType ? GrClampType::kManual
                                                     : GrClampType::kNone;
}

// Consider a color type "wider" than n if it has more than n bits for any its
// representable channels.
static constexpr bool GrColorTypeIsWiderThan(GrColorType colorType, int n) {
  SkASSERT(n > 0);
  auto desc = GrGetColorTypeDesc(colorType);
  return (desc.r() && desc.r() > n) || (desc.g() && desc.g() > n) ||
         (desc.b() && desc.b() > n) || (desc.a() && desc.a() > n) ||
...
// In general we try to not mix CompressionType and ColorType, but currently
// SkImage still requires an SkColorType even for CompressedTypes so we need
// some conversion.
