...
static_assert(PNG_FILTER_NONE == (int)SkPngEncoder::FilterFlag::kNone,
              "Skia libpng filter err.");
static_assert(PNG_FILTER_SUB == (int)SkPngEncoder::FilterFlag::kSub,
              "Skia libpng filter err.");
static_assert(PNG_FILTER_UP == (int)SkPngEncoder::FilterFlag::kUp,
              "Skia libpng filter err.");
static_assert(PNG_FILTER_AVG == (int)SkPngEncoder::FilterFlag::kAvg,
              "Skia libpng filter err.");
static_assert(PNG_FILTER_PAETH == (int)SkPngEncoder::FilterFlag::kPaeth,
              "Skia libpng filter err.");
static_assert(PNG_ALL_FILTERS == (int)SkPngEncoder::FilterFlag::kAll,
              "Skia libpng filter err.");
...
class SkPngEncoderMgr final : SkNoncopyable {
 ...
 private:
  SkPngEncoderMgr(png_structp pngPtr, png_infop infoPtr)
      : fPngPtr(pngPtr), fInfoPtr(infoPtr) {}
...
std::unique_ptr<SkPngEncoderMgr> SkPngEncoderMgr::Make(SkWStream* stream) {
  png_structp pngPtr = png_create_write_struct(PNG_LIBPNG_VER_STRING, nullptr,
                                               sk_error_fn, nullptr);
  if (!pngPtr) {
    return nullptr;
  }
...
bool SkPngEncoderMgr::setHeader(const SkImageInfo& srcInfo,
  ...
  switch (srcInfo.colorType()) {
    ...
    case kRGBA_F32_SkColorType:
      ...
      bitDepth = 16;
      pngColorType =
          srcInfo.isOpaque() ? PNG_COLOR_TYPE_RGB : PNG_COLOR_TYPE_RGB_ALPHA;
      fPngBytesPerPixel = 8;
      break;
  ...
  png_set_IHDR(fPngPtr, fInfoPtr, srcInfo.width(), srcInfo.height(), bitDepth,
               pngColorType, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE,
  ...
  if (comments != nullptr) {
    ...
    for (int i = 0; i < comments->count() / 2; ++i) {
      ...
      // and we don't have to provide text_length and other fields as we're
      // providing 0-terminated c_str with PNG_TEXT_COMPRESSION_NONE (no
      // compression, no itxt).
...
std::unique_ptr<SkEncoder> SkPngEncoder::Make(SkWStream* dst,
  ...
  return std::unique_ptr<SkPngEncoder>(
      new SkPngEncoder(std::move(encoderMgr), src));
}

SkPngEncoder::SkPngEncoder(std::unique_ptr<SkPngEncoderMgr> encoderMgr,
                           const SkPixmap& src)
...
bool SkPngEncoder::onEncodeRows(int numRows) {
  ...
  for (int y = 0; y < numRows; y++) {
    ...
    fEncoderMgr->proc()((char*)fStorage.get(), (const char*)srcRow,
                        fSrc.width(),
...
bool SkPngEncoder::Encode(SkWStream* dst, const SkPixmap& src,
                          const Options& options) {
