...
size_t SkImageInfo::computeByteSize(size_t rowBytes) const {
  ...
  // The CPU backend implements some memory operations on images using
  // instructions that take a signed 32-bit offset from the base. If we ever
  // make an image larger than that, overflow can cause us to read/write memory
  // that starts 2GB *before* the buffer. (crbug.com/1264705)
...
SkImageInfo SkImageInfo::Make(int width, int height, SkColorType ct,
                              SkAlphaType at) {
...
SkImageInfo SkImageInfo::Make(SkISize dimensions, SkColorType ct,
                              SkAlphaType at) {
...
SkImageInfo SkImageInfo::MakeS32(int width, int height, SkAlphaType at) {
  return SkImageInfo({width, height},
                     {kN32_SkColorType, at, SkColorSpace::MakeSRGB()});
}
...
SkImageInfo SkImageInfo::MakeN32Premul(int width, int height,
                                       sk_sp<SkColorSpace> cs) {
  return Make({width, height}, kN32_SkColorType, kPremul_SkAlphaType,
              std::move(cs));
}
...
SkImageInfo SkImageInfo::MakeA8(int width, int height) {
  return Make({width, height}, kAlpha_8_SkColorType, kPremul_SkAlphaType,
              nullptr);
}
...
SkImageInfo SkImageInfo::MakeUnknown(int width, int height) {
  return Make({width, height}, kUnknown_SkColorType, kUnknown_SkAlphaType,
              nullptr);
}
...
bool SkWritePixelsRec::trim(int dstWidth, int dstHeight) {
  ...
  // we negate and add them so UBSAN (pointer-overflow) doesn't get confused.
  fPixels =
      ((const char*)fPixels + -y * fRowBytes + -x * fInfo.bytesPerPixel());
  // the intersect may have shrunk info's logical size
