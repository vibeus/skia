...
void SkRasterPipeline::append(StockStage stage, void* ctx) {
  ...
  SkASSERT(stage !=
           unbounded_uniform_color);     // Please use append_constant_color().
  SkASSERT(stage != set_rgb);            // Please use append_set_rgb().
  SkASSERT(stage != unbounded_set_rgb);  // Please use append_set_rgb().
  SkASSERT(stage !=
           clamp_gamut);  // Please use append_gamut_clamp_if_normalized().
...
void SkRasterPipeline::append_set_rgb(SkArenaAlloc* alloc, const float rgb[3]) {
  ...
  if (0 <= rgb[0] && rgb[0] <= 1 && 0 <= rgb[1] && rgb[1] <= 1 && 0 <= rgb[2] &&
      rgb[2] <= 1) {
    stage = set_rgb;
  }
...
void SkRasterPipeline::append_constant_color(SkArenaAlloc* alloc,
                                             const float rgba[4]) {
  ...
  if (rgba[0] == 0 && rgba[1] == 0 && rgba[2] == 0 && rgba[3] == 1) {
    ...
    // while unbounded_uniform_color supports out-of-range colors too but not
    // lowp.
    if (0 <= rgba[0] && rgba[0] <= rgba[3] && 0 <= rgba[1] &&
        rgba[1] <= rgba[3] && 0 <= rgba[2] && rgba[2] <= rgba[3]) {
...
void SkRasterPipeline::append_matrix(SkArenaAlloc* alloc,
...
void SkRasterPipeline::append_load(SkColorType ct,
...
void SkRasterPipeline::append_load_dst(SkColorType ct,
                                       const SkRasterPipeline_MemoryCtx* ctx) {
  switch (ct) {
    case kUnknown_SkColorType:
      SkASSERT(false);
    ...
    case kSRGBA_8888_SkColorType:
      // TODO: We could remove the double-swap if we had _dst versions of all
      // the TF stages
      this->append(load_8888_dst, ctx);
...
void SkRasterPipeline::append_store(SkColorType ct,
                                    const SkRasterPipeline_MemoryCtx* ctx) {
  switch (ct) {
    case kUnknown_SkColorType:
      SkASSERT(false);
...
void SkRasterPipeline::append_transfer_function(
  ...
  switch (classify_transfer_fn(tf)) {
    ...
    case TFKind::sRGBish_TF:
      if (tf.a == 1 && tf.b == 0 && tf.c == 0 && tf.d == 0 && tf.e == 0 &&
          tf.f == 0) {
...
// scenario of being able to store a logical color channel > 1.0 when alpha
// < 1.0. Most software that works with normalized premul values expect r,g,b
// channels all <= a.
//
...
void SkRasterPipeline::append_gamut_clamp_if_normalized(
    const SkImageInfo& info) {
  if (info.alphaType() == kPremul_SkAlphaType &&
      SkColorTypeIsNormalized(info.colorType())) {
...
SkRasterPipeline::StartPipelineFn SkRasterPipeline::build_pipeline(
  ...
  if (!gForceHighPrecisionRasterPipeline) {
    // We'll try to build a lowp pipeline, but if that fails fallback to a highp
    // float pipeline.
    void** reset_point = ip;

    // Stages are stored backwards in fStages, so we reverse here, back to
    // front.
...
void SkRasterPipeline::run(size_t x, size_t y, size_t w, size_t h) const {
  ...
  // Best to not use fAlloc here... we can't bound how often run() will be
  // called.
