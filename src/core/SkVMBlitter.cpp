...
// rather than by the Shader, which follow this struct in the skvm::Uniforms
// buffer.
struct BlitterUniforms {
  int right;  // First device x + blit run length n, used to get device x
              // coordinate.
...
static skvm::Coord device_coord(skvm::Builder* p, skvm::Uniforms* uniforms) {
  skvm::I32 dx =
                p->uniform32(uniforms->base, offsetof(BlitterUniforms, right)) -
                p->index(),
...
struct NoopColorFilter : public SkColorFilterBase {
  skvm::Color onProgram(skvm::Builder*, skvm::Color c, const SkColorInfo&,
...
struct SpriteShader : public SkShaderBase {
  ...
  skvm::Color onProgram(skvm::Builder* p, skvm::Coord /*device*/,
                        skvm::Coord /*local*/, skvm::Color /*paint*/,
...
struct DitherShader : public SkShaderBase {
  ...
  skvm::Color onProgram(skvm::Builder* p, skvm::Coord device, skvm::Coord local,
    ...
    skvm::Color c = as_SB(fShader)->program(p, device, local, paint, matrices,
                                            localM, dst, uniforms, alloc);
    if (!c) {
      return {};
    }
    ...
    skvm::I32 M = shl(Y & 1, 5) | shl(X & 1, 4) | shl(Y & 2, 2) |
                  shl(X & 2, 1) | shr(Y & 4, 1) | shr(X & 4, 2);
    ...
    // rate could be a uniform, but since it's based on the destination
    // SkColorType, we can bake it in without hurting the cache hit rate.
...
bool SkVMBlitter::Key::operator==(const Key& that) const {
  return this->shader == that.shader && this->clip == that.clip &&
         this->blender == that.blender && this->colorSpace == that.colorSpace &&
...
SkVMBlitter::Params SkVMBlitter::EffectiveParams(
  ...
  // Sprites take priority over any shader.  (There's rarely one set, and it's
  // meaningless.)
  if (sprite) {
  ...
  if (as_BB(blender)->asBlendMode() == SkBlendMode::kSrcOver &&
      shader->isOpaque()) {
...
skvm::Color SkVMBlitter::DstColor(skvm::Builder* p, const Params& params) {
  skvm::PixelFormat dstFormat =
      skvm::SkColorType_to_PixelFormat(params.dst.colorType());
  skvm::Ptr dst_ptr =
      p->varying(SkColorTypeBytesPerPixel(params.dst.colorType()));
...
void SkVMBlitter::BuildProgram(skvm::Builder* p, const Params& params,
  ...
  // source. Subsequent arguments depend on params.coverage:
  ...
  // See note about arguments above: a SpriteShader will call p->arg() once
  // during program().
  skvm::Color src =
      as_SB(params.shader)
          ->program(p, device, /*local=*/device, paint, params.matrices,
  ...
  // GL clamps all its color channels to limits of the format just before the
  // blend step (~here).
  // TODO: Below, we also clamp after the blend step. If we can prove that none
  // of the work here (especially blending, for built-in blend modes) will
  // produce colors outside [0, 1] we may be able to skip the second clamp. For
  // now, we clamp twice.
  ...
  if (params.dst.isOpaque()) {
    // When a destination is known opaque, we may assume it both starts and
    // stays fully opaque, ignoring any math that disagrees.  This sometimes
    // trims a little work.
    dst.a = p->splat(1.0f);
  ...
  switch (params.coverage) {
    ...
    case Coverage::MaskLCD16: {
      skvm::PixelFormat fmt =
          skvm::SkColorType_to_PixelFormat(kRGB_565_SkColorType);
  ...
  // The math for some blend modes lets us fold coverage into src before the
  // blend, which is simpler than the canonical post-blend lerp().
...
SkVMBlitter::Key SkVMBlitter::CacheKey(const Params& params,
                                       skvm::Uniforms* uniforms,
                                       SkArenaAlloc* alloc, bool* ok) {
  ...
  auto hash_shader = [&](skvm::Builder& p, const sk_sp<SkShader>& shader,
    ...
    uint64_t hash = 0;
    *outColor =
        sb->program(&p, device, /*local=*/device, paint, params.matrices,
                    /*localM=*/nullptr, params.dst, uniforms, alloc);
    if (*outColor) {
      hash = p.hash();
      ...
      // called std::swap(*r,*b)... it draws differently, but p.hash() is
      // unchanged. We'll fold the hash of their IDs in order to disambiguate.
      const skvm::Val outputs[] = {outColor->r.id, outColor->g.id,
                                   outColor->b.id, outColor->a.id};
  ...
  // Use this builder for shader, clip and blender, so that color objects that
  // pass from one to the other all 'make sense' -- i.e. have the same builder
  // and/or have meaningful values for the hash.
  //
  // Question: better if we just pass in mock uniform colors, so we don't need
  // to
  //           explicitly use the output color from one stage as input to
  //           another?
  //
  skvm::Builder p;
  ...
  if (auto bm = as_BB(params.blender)->asBlendMode()) {
    ...
    skvm::Color outColor =
        blender->program(&p, src, dst, params.dst, uniforms, alloc);
    if (outColor) {
      blendHash = p.hash();
...
SkVMBlitter::SkVMBlitter(const SkPixmap& device, const SkPaint& paint,
                         const SkPixmap* sprite, SkIPoint spriteOffset,
                         const SkMatrixProvider& matrices, sk_sp<SkShader> clip,
                         bool* ok)
    : fDevice(device),
      ...
      fParams(
          EffectiveParams(device, sprite, paint, matrices, std::move(clip))),
...
SkVMBlitter::~SkVMBlitter() {
  if (fStoreToCache) {
    if (SkLRUCache<Key, skvm::Program>* cache = TryAcquireProgramCache()) {
      auto cache_program = [&](SkTLazy<skvm::Program>& program,
                               Coverage coverage) {
        if (program.isValid() && !program->hasTraceHooks()) {
          cache->insert_or_update(fKey.withCoverage(coverage),
                                  std::move(*program));
        }
      };
...
SkLRUCache<SkVMBlitter::Key, skvm::Program>*
SkVMBlitter::TryAcquireProgramCache() {
#if defined(SKVM_JIT)
...
SkString SkVMBlitter::DebugName(const Key& key) {
  return SkStringPrintf("Shader-%" PRIx64 "_Clip-%" PRIx64 "_Blender-%" PRIx64
                        ...
                        key.shader, key.clip, key.blender, key.colorSpace,
                        key.colorType, key.alphaType, key.coverage);
}
...
skvm::Program* SkVMBlitter::buildProgram(Coverage coverage) {
  ...
  // and more natural to rebuild fUniforms than to emit them into a temporary
  // buffer. fUniforms should reuse the exact same memory, so this is very
  // cheap.
  ...
  SkASSERTF(fUniforms.buf.size() == prev, "%zu, prev was %zu",
            fUniforms.buf.size(), prev);
  ...
  if ((false)) {
    ...
    if (0 == total++) {
      atexit([] {
        SkDebugf("SkVMBlitter compiled %d programs, %d without JIT.\n",
...
void SkVMBlitter::blitAntiH(int x, int y, const SkAlpha cov[],
                            const int16_t runs[]) {
  ...
  for (int16_t run = *runs; run > 0; run = *runs) {
    ...
    if (coverage != 0x00) {
      ...
      if (coverage == 0xFF) {
        ...
        if (sprite) {
          blit_anti_h->eval(run, fUniforms.buf.data(), fDevice.addr(x, y),
                            sprite, &covF);
        } else {
          blit_anti_h->eval(run, fUniforms.buf.data(), fDevice.addr(x, y),
                            &covF);
...
SkVMBlitter* SkVMBlitter::Make(const SkPixmap& device, const SkPaint& paint,
                               const SkMatrixProvider& matrices,
                               SkArenaAlloc* alloc, sk_sp<SkShader> clip) {
  bool ok = true;
  SkVMBlitter* blitter =
      alloc->make<SkVMBlitter>(device, paint, /*sprite=*/nullptr,
                               SkIPoint{0, 0}, matrices, std::move(clip), &ok);
...
SkVMBlitter* SkVMBlitter::Make(const SkPixmap& device, const SkPaint& paint,
  ...
  auto blitter = alloc->make<SkVMBlitter>(
      device, paint, &sprite, SkIPoint{left, top}, SkMatrixProvider{SkMatrix{}},
      std::move(clip), &ok);
  return ok ? blitter : nullptr;
}
