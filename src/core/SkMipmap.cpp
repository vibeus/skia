...
// It controls how we expand a pixel into a large type, with space between each
// component, so we can then perform our simple filter (either box or triangle)
// and store the intermediates in the expanded type.
//
...
struct ColorTypeFilter_565 {
  ...
  static uint16_t Compact(uint32_t x) {
    return ((x & ~SK_G16_MASK_IN_PLACE) & 0xFFFF) |
           ((x >> 16) & SK_G16_MASK_IN_PLACE);
  }
};
...
struct ColorTypeFilter_1010102 {
  ...
  static uint64_t Expand(uint64_t x) {
    return (((x)&0x3ff)) | (((x >> 10) & 0x3ff) << 20) |
           (((x >> 20) & 0x3ff) << 40) | (((x >> 30) & 0x3) << 60);
  }
  static uint32_t Compact(uint64_t x) {
    return (((x)&0x3ff)) | (((x >> 20) & 0x3ff) << 10) |
           (((x >> 40) & 0x3ff) << 20) | (((x >> 60) & 0x3) << 30);
  }
};
...
//
//  To produce each mip level, we need to filter down by 1/2 (e.g. 100x100 ->
//  50,50) If the starting dimension is odd, we floor the size of the lower
//  level (e.g. 101 -> 50) In those (odd) cases, we use a triangle filter, with
//  1-pixel overlap between samplings, else for even cases, we just use a 2x box
//  filter.
//
//  This produces 4 possible isotropic filters: 2x2 2x3 3x2 3x3 where WxH
//  indicates the number of src pixels we need to sample in each dimension to
//  produce 1 dst pixel.
//
...
SkMipmap* SkMipmap::Build(const SkPixmap& src, SkDiscardableFactoryProc fact,
  ...
  for (int currentMipLevel = countLevels; currentMipLevel >= 0;
       currentMipLevel--) {
    SkISize mipSize =
        ComputeLevelSize(src.width(), src.height(), currentMipLevel);
  ...
  // Depending on architecture and other factors, the pixel data alignment may
  // need to be as large as 8 (for F16 pixels). See the comment on
  // SkMipmap::Level.
  ...
  for (int i = 0; i < countLevels; ++i) {
    ...
    // We make the Info w/o any colorspace, since that storage is not under our
    // control, and will not be deleted in a controlled fashion. When the caller
    // is given the pixmap for a given level, we augment this pixmap with fCS
    // (which we do manage).
    new (&levels[i].fPixmap)
        SkPixmap(SkImageInfo::Make(width, height, ct, at), addr, rowBytes);
...
// Returns fractional level value. floor(level) is the index of the larger
// level. < 0 means failure.
...
#else
  // Ideally we'd pick the smaller scale, to match Ganesh.  But ignoring one of
  // the scales can produce some atrocious results, so for now we use the
  // geometric mean. (https://bugs.chromium.org/p/skia/issues/detail?id=4863)
...
bool SkMipmap::Deserialize(SkMipmapBuilder* builder, const void* data,
                           size_t length) {
