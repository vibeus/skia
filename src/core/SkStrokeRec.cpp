...
bool SkStrokeRec::applyToPathChopped(std::vector<SkPathBuilder>* result, const SkPath& src, size_t chop_verbs) const {
    if (fWidth <= 0) {  // hairline or fill
        return false;
    }
    ...
    stroker.setResScale(fResScale);
#endif
    stroker.strokePathChopped(src, result, chop_verbs);
    return true;
}
