...
class SkRasterPipelineBlitter final : public SkBlitter {
 public:
  // This is our common entrypoint for creating the blitter once we've sorted
  // out shaders.
 ...
 private:
  ...
  void (*fMemset2D)(SkPixmap*, int x, int y, int w, int h,
                    uint64_t color) = nullptr;
  uint64_t fMemsetColor =
      0;  // Big enough for largest memsettable dst format, F16.

  // Built lazily on first use.
  std::function<void(size_t, size_t, size_t, size_t)> fBlitRect, fBlitAntiH,
...
SkBlitter* SkCreateRasterPipelineBlitter(const SkPixmap& dst,
  ...
  SkColorSpaceXformSteps(sk_srgb_singleton(), kUnpremul_SkAlphaType, dstCS,
  ...
  if (shader->appendStages({&shaderPipeline, alloc, dstCT, dstCS, paint,
                            nullptr, matrixProvider})) {
...
SkBlitter* SkCreateRasterPipelineBlitter(const SkPixmap& dst,
                                         ...
                                         sk_sp<SkShader> clipShader) {
  bool is_constant = false;  // If this were the case, it'd be better to just
                             // set a paint color.
  return SkRasterPipelineBlitter::Create(dst, paint, alloc, shaderPipeline,
                                         is_opaque, is_constant, clipShader);
}
...
SkBlitter* SkRasterPipelineBlitter::Create(
  ...
  // This is the common front of the full blit pipelines, each constructed
  // lazily on first use. The full blit pipelines handle reading and writing the
  // dst, blending, coverage, dithering.
  ...
  if (clipShader) {
    ...
    SkStageRec rec = {
        clipP, alloc, clipCT, clipCS, clipPaint, nullptr, clipMatrixProvider};
  ...
  // Not all formats make sense to dither (think, F16).  We set their dither
  // rate to zero.  We only dither non-constant shaders, so is_constant won't
  // change here.
  ...
  // We're logically done here.  The code between here and return blitter is all
  // optimization.

  // A pipeline that's still constant here can collapse back into a constant
  // color.
  if (is_constant) {
  ...
  // When we're drawing a constant color in Src mode, we can sometimes just
  // memset. (The previous two optimizations help find more opportunities for
  // this one.)
  if (is_constant && blitter->fBlend == SkBlendMode::kSrc) {
    // Run our color pipeline all the way through to produce what we'd memset
    // when we can. Not all blits can memset, so we need to keep colorPipeline
    // too.
    ...
    switch (blitter->fDst.shiftPerPixel()) {
      case 0:
        blitter->fMemset2D = [](SkPixmap* dst, int x, int y, int w, int h,
                                uint64_t c) {
      ...
      case 1:
        blitter->fMemset2D = [](SkPixmap* dst, int x, int y, int w, int h,
                                uint64_t c) {
          SkOpts::rect_memset16(dst->writable_addr16(x, y), c, w,
                                dst->rowBytes(), h);
        };
        break;
      ...
      case 2:
        blitter->fMemset2D = [](SkPixmap* dst, int x, int y, int w, int h,
                                uint64_t c) {
          SkOpts::rect_memset32(dst->writable_addr32(x, y), c, w,
                                dst->rowBytes(), h);
        };
        break;
      ...
      case 3:
        blitter->fMemset2D = [](SkPixmap* dst, int x, int y, int w, int h,
                                uint64_t c) {
          SkOpts::rect_memset64(dst->writable_addr64(x, y), c, w,
                                dst->rowBytes(), h);
        };
        break;
...
void SkRasterPipelineBlitter::blitRectWithTrace(int x, int y, int w, int h,
                                                bool trace) {
  if (fMemset2D) {
...
void SkRasterPipelineBlitter::blitAntiH(int x, int y, const SkAlpha aa[],
                                        const int16_t runs[]) {
  if (!fBlitAntiH) {
    SkRasterPipeline p(fAlloc);
    p.extend(fColorPipeline);
...
void SkRasterPipelineBlitter::blitMask(const SkMask& mask,
  ...
  auto extract_mask_plane = [&mask](int plane,
    ...
    // Select the right mask plane.  Usually plane == 0 and this is just
    // mask.fImage.
    auto ptr = (uintptr_t)mask.fImage + plane * mask.computeImageSize();

    // Update ctx to point "into" this current mask, but lined up with fDstPtr
    // at (0,0). This sort of trickery upsets UBSAN (pointer-overflow) so our
    // ptr must be a uintptr_t. mask.fRowBytes is a uint32_t, which would break
    // our addressing math on 64-bit builds.
