...
static void* alloc_jit_buffer(size_t* len) {
  ...
  // mprotect doesn't round up for you, and instead requires *len is at page
  // granularity.
...
//
// Similarly, we can't get ASAN's checks unless we let it instrument our
// interpreter.
...
// skslc needs to link against this module (for the VM code generator). This
// module pulls in color-space code, but attempting to add those transitive
// dependencies to skslc gets out of hand. So we terminate the chain here with
// stub functions. Note that skslc's usage of SkVM never cares about color
// management.
skvm::F32 sk_program_transfer_fn(skvm::F32 v, TFKind tf_kind, skvm::F32 G,
                                 skvm::F32 A, skvm::F32 B, skvm::F32 C,
                                 skvm::F32 D, skvm::F32 E, skvm::F32 F) {
  return v;
}
...
static void write_one_instruction(Val id, const OptimizedInstruction& inst,
                                  SkWStream* o) {
  Op op = inst.op;
...
std::vector<Instruction> eliminate_dead_code(std::vector<Instruction> program,
  ...
  //   - remap IDs in live Instructions to what they'll be once dead
  ...
  auto it = std::remove_if(program.begin(), program.end(),
...
std::vector<OptimizedInstruction> finalize(
  ...
  // Each Instruction's inputs need to live at least until that Instruction
  // issues.
  for (Val id = 0; id < (Val)optimized.size(); id++) {
    OptimizedInstruction& inst = optimized[id];
    for (Val arg : {inst.x, inst.y, inst.z, inst.w}) {
      // (We're walking in order, so this is the same as max()ing with the
      // existing Val.)
      if (arg != NA) {
  ...
  for (OptimizedInstruction& inst : optimized) {
    ...
    // If any of an instruction's inputs can't be hoisted, it can't be hoisted
    // itself.
    if (inst.can_hoist) {
      for (Val arg : {inst.x, inst.y, inst.z, inst.w}) {
        if (arg != NA) {
  ...
  // Extend the lifetime of any hoisted value that's used in the loop to
  // infinity.
  for (OptimizedInstruction& inst : optimized) {
...
std::vector<OptimizedInstruction> Builder::optimize(
    viz::Visualizer* visualizer) const {
...
Program Builder::done(const char* debug_name, bool allow_jit,
  ...
  return {optimized, std::move(visualizer), fStrides, fTraceHooks, debug_name,
          allow_jit};
}

uint64_t Builder::hash() const {
  uint32_t lo = SkOpts::hash(fProgram.data(),
                             fProgram.size() * sizeof(Instruction), 0),
           hi = SkOpts::hash(fProgram.data(),
                             fProgram.size() * sizeof(Instruction), 1);
...
bool operator==(const Instruction& a, const Instruction& b) {
  return a.op == b.op && a.x == b.x && a.y == b.y && a.z == b.z && a.w == b.w &&
         a.immA == b.immA && a.immB == b.immB && a.immC == b.immC;
}

uint32_t InstructionHash::operator()(const Instruction& inst,
                                     uint32_t seed) const {
...
Val Builder::push(Instruction inst) {
  ...
  // if we've already seen this exact Instruction, use it instead of creating a
  // new one.
  //
  // But we never dedup loads or stores: an intervening store could change that
  // memory. Uniforms and gathers touch only uniform memory, so they're fine to
  // dedup, and index is varying but doesn't touch memory, so it's fine to dedup
  // too.
...
void Builder::trace_var(int traceHookID, I32 mask, I32 traceMask, int slot,
                        I32 val) {
  SkASSERT(traceHookID >= 0);
  SkASSERT(traceHookID < (int)fTraceHooks.size());
  if (!this->mergeMasks(mask, traceMask)) {
  ...
  (void)push(Op::trace_var, mask.id, traceMask.id, val.id, NA, traceHookID,
             slot);
}
void Builder::trace_enter(int traceHookID, I32 mask, I32 traceMask, int fnIdx) {
  ...
  (void)push(Op::trace_enter, mask.id, traceMask.id, NA, NA, traceHookID,
             fnIdx);
}
...
void Builder::trace_scope(int traceHookID, I32 mask, I32 traceMask, int delta) {
  ...
  (void)push(Op::trace_scope, mask.id, traceMask.id, NA, NA, traceHookID,
             delta);
}
...
I32 Builder::array32(UPtr ptr, int offset, int index) {
  return {this, push(Op::array32, NA, NA, NA, NA, ptr.ix, offset,
                     index * sizeof(int))};
}
...
//
//     bool equiv(float x, float y) { return (x == y) || (isnanf(x) &&
//     isnanf(y)); }
//
//     unsigned bits = 0;
//     do {
...
F32 Builder::add(F32 x, F32 y) {
  ...
  if (fFeatures.fma) {
    if (fProgram[x.id].op == Op::mul_f32) {
      return {this, this->push(Op::fma_f32, fProgram[x.id].x, fProgram[x.id].y,
                               y.id)};
    }
    if (fProgram[y.id].op == Op::mul_f32) {
      return {this, this->push(Op::fma_f32, fProgram[y.id].x, fProgram[y.id].y,
                               x.id)};
    }
  }
...
F32 Builder::sub(F32 x, F32 y) {
  ...
  if (fFeatures.fma) {
    if (fProgram[x.id].op == Op::mul_f32) {
      return {this, this->push(Op::fms_f32, fProgram[x.id].x, fProgram[x.id].y,
                               y.id)};
    }
    if (fProgram[y.id].op == Op::mul_f32) {
      return {this, this->push(Op::fnma_f32, fProgram[y.id].x, fProgram[y.id].y,
                               x.id)};
    }
  }
...
F32 Builder::approx_powf(F32 x, F32 y) {
  // TODO: assert this instead?  Sometimes x is very slightly negative.  See
  // skia:10210.
  x = max(0.0f, x);
...
//
// This is a good approximation only for 0 <= x <= pi, so we use symmetries to
// get radians into that range first.
//
...
/*  "GENERATING ACCURATE VALUES FOR THE TANGENT FUNCTION"
    ...
    3. Our polynomial approximation is best near zero, so we use the following
   identity tan(x) + tan(y) tan(x + y) = ----------------- 1 - tan(x)*tan(y)
       tan(PI/4) = 1
...
F32 Builder::approx_tan(F32 x) {
  ...
  // shift to 0...Pi, scale 1/Pi to get into 0...1, then fract, scale-up,
  // shift-back
...
F32 Builder::approx_asin(F32 x) {
  ...
  x = select(neg, -x, x);
  x = SK_ScalarPI / 2 -
      sqrt(1 - x) * poly(x, -0.0187293f, 0.0742610f, -0.2121144f, 1.5707288f);
  x = select(neg, -x, x);
  return x;
}
...
static F32 approx_atan_unit(F32 x) {
  ...
  return poly(x, 0.14130025741326729f, -0.34312835980675116f,
              -0.016172900528248768f, 1.0037696976200385f,
...
I32 Builder::bit_and(I32 x, I32 y) {
  ...
  if (Val notX = this->holdsBitNot(x.id);
      notX != NA) {  // (~x & y) == bit_clear(y, ~x)
    return bit_clear(y, {this, notX});
  }
  if (Val notY = this->holdsBitNot(y.id);
      notY != NA) {  // (x & ~y) == bit_clear(x, ~y)
...
PixelFormat SkColorType_to_PixelFormat(SkColorType ct) {
  auto UNORM = PixelFormat::UNORM, SRGB = PixelFormat::SRGB,
  ...
  switch (ct) {
    ...
    // case kBGR_888_SkColorType:
    // return {UNORM, 8, 8, 8, 0, 0, 8, 16, 24};
...
static int byte_size(PixelFormat f) {
  ...
  int bits =
      std::max(f.r_bits + f.r_shift,
               std::max(f.g_bits + f.g_shift,
                        std::max(f.b_bits + f.b_shift, f.a_bits + f.a_shift)));
  // Round up to bytes.
  return (bits + 7) / 8;
}
...
static Color unpack(PixelFormat f, I32 x) {
  ...
  auto from_srgb = [](int bits, I32 channel) -> F32 {
    ...
    return sk_program_transfer_fn(
        v, sRGBish_TF, v->splat(tf->g), v->splat(tf->a), v->splat(tf->b),
        v->splat(tf->c), v->splat(tf->d), v->splat(tf->e), v->splat(tf->f));
  };
...
static void split_disjoint_8byte_format(PixelFormat f, PixelFormat* lo,
                                        PixelFormat* hi) {
  SkASSERT(byte_size(f) == 8);
  // We assume some of the channels are in the low 32 bits, some in the high 32
  // bits. The assert on byte_size(lo) will trigger if this assumption is
  // violated.
  *lo = f;
  if (f.r_shift >= 32) {
    lo->r_bits = 0;
...
// though, TODO, we could generalize that to any swizzle, and to allow UNORM
// too.
...
static I32 pack32(PixelFormat f, Color c) {
  ...
  auto to_srgb = [](int bits, F32 v) {
    ...
    return to_unorm(bits, sk_program_transfer_fn(
                              v, sRGBish_TF, v->splat(tf->g), v->splat(tf->a),
                              v->splat(tf->b), v->splat(tf->c), v->splat(tf->d),
                              v->splat(tf->e), v->splat(tf->f)));
  };
...
void Builder::store(PixelFormat f, Ptr ptr, Color c) {
  ...
  if (f.r_bits == f.g_bits && f.g_bits == f.b_bits && f.r_shift == f.g_shift &&
      f.g_shift == f.b_shift) {
    // TODO: pull these coefficients from an SkColorSpace?  This is sRGB
    // luma/luminance.
  ...
  switch (byte_size(f)) {
    ...
    case 16: {
      assert_16byte_is_rgba_f32(f);
      store128(ptr, pun_to_I32(c.r), pun_to_I32(c.g), pun_to_I32(c.b),
               pun_to_I32(c.a));
      break;
    }
    default:
...
void Builder::unpremul(F32* r, F32* g, F32* b, F32 a) {
  ...
  // If a is 0, so are *r,*g,*b, so set invA to 0 to avoid 0*inf=NaN (instead
  // 0*0 = 0).
...
HSLA Builder::to_hsla(Color c) {
  F32 mx = max(max(c.r, c.g), c.b), mn = min(min(c.r, c.g), c.b), d = mx - mn,
      invd = 1.0f / d, g_lt_b = select(c.g < c.b, splat(6.0f), splat(0.0f));

  F32 h =
      (1 / 6.0f) * select(mx == mn, 0.0f,
...
static void clip_color(skvm::F32* r, skvm::F32* g, skvm::F32* b, skvm::F32 a) {
  F32 mn = min(*r, min(*g, *b)), mx = max(*r, max(*g, *b)),
      lu = luminance(*r, *g, *b);
...
Color Builder::blend(SkBlendMode mode, Color src, Color dst) {
  ...
  switch (mode) {
    ...
    case SkBlendMode::kSrcATop:
      return apply_rgba(
          [&](auto s, auto d) { return mma(s, dst.a, d, 1 - src.a); });

    case SkBlendMode::kXor:
      return apply_rgba(
          [&](auto s, auto d) { return mma(s, 1 - dst.a, d, 1 - src.a); });
    ...
    case SkBlendMode::kExclusion:
      return apply_rgb_srcover_a(
          [&](auto s, auto d) { return s + (d - two(s * d)); });
    ...
    case SkBlendMode::kColorBurn:
      return apply_rgb_srcover_a([&](auto s, auto d) {
        ...
        return select(d == dst.a, s * (1 - dst.a) + d,
                      select(is_finite(burn), burn, d * (1 - src.a) + s));
      });
    ...
    case SkBlendMode::kColorDodge:
      return apply_rgb_srcover_a([&](auto s, auto d) {
        ...
        return select(d == 0.0f, s * (1 - dst.a) + d,
                      select(is_finite(dodge), dodge, d * (1 - src.a) + s));
      });

    case SkBlendMode::kHardLight:
      return apply_rgb_srcover_a([&](auto s, auto d) {
        return mma(s, 1 - dst.a, d, 1 - src.a) +
               select(two(s) <= src.a, two(s * d),
    ...
    case SkBlendMode::kSoftLight:
      return apply_rgb_srcover_a([&](auto s, auto d) {
        ...
        auto darkSrc = d * ((s2 - src.a) * (1 - m) + src.a),
            ...
            liteDst = sqrt(m) - m,
             // Used in 2 or 3?
            liteSrc = dst.a * (s2 - src.a) *
                          select(4 * d <= dst.a, darkDst, liteDst) +
                      d * src.a;
        return s * (1 - dst.a) + d * (1 - src.a) +
               select(s2 <= src.a, darkSrc, liteSrc);
      });
...
static VEX vex(
  ...
  if (X == 0 && B == 0 && WE == 0 && map == 0b00001) {
    ...
    vex.bytes[0] = 0xc5;
    vex.bytes[1] =
        (pp & 3) << 0 | (L & 1) << 2 | (~vvvv & 15) << 3 | (~(int)R & 1) << 7;
  } else {
    // We could use this 3-byte VEX prefix all the time if we like.
    vex.len = 3;
    vex.bytes[0] = 0xc4;
    vex.bytes[1] = (map & 31) << 0 | (~(int)B & 1) << 5 | (~(int)X & 1) << 6 |
                   (~(int)R & 1) << 7;
    vex.bytes[2] =
        (pp & 3) << 0 | (L & 1) << 2 | (~vvvv & 15) << 3 | (WE & 1) << 7;
  }
  return vex;
}
...
// Shift instructions encode their opcode extension as "dst", dst as x, and x as
// y.
...
void Assembler::vpermq(Ymm dst, Operand x, int imm) {
  // A bit unusual among the instructions we use, this is 64-bit operation, so
  // we set W.
...
void Assembler::op(int prefix, int map, int opcode, int dst, int x, Operand y,
                   W w, L l) {
  switch (y.kind) {
    case Operand::REG: {
    ...
    case Operand::MEM: {
      ...
      // This means we have to use an SIB byte if we want to use rsp as a base
      // register.
      const Mem& m = y.mem;
    ...
    case Operand::LABEL: {
      // IP-relative addressing uses Mod::Indirect with the R/M encoded as-if
      // rbp or r13.
      const int rip = rbp;
...
void Assembler::jump(uint8_t condition, Label* l) {
  ...
  //    7?     one-byte-disp
  //    0F 8? four-byte-disp
  // We always use the near displacement to make updating labels simpler (no
  // resizing).
  this->byte(0x0f);
  this->byte(condition);
...
void Assembler::jmp(Label* l) {
  // Like above in jump(), we could use 8-bit displacement here, but always use
  // 32-bit.
  this->byte(0xe9);
  this->word(this->disp32(l));
}
...
void Assembler::op(uint32_t hi, V m, uint32_t lo, V n, V d) {
  this->word((hi & mask(11)) << 21 | (m & mask(5)) << 16 |
             (lo & mask(6)) << 10 | (n & mask(5)) << 5 | (d & mask(5)) << 0);
}
...
void Assembler::add(X d, X n, X m, Shift shift, int imm6) {
  ...
  int imm = (imm6 & mask(6)) << 0 | (m & mask(5)) << 6 | (0 & mask(1)) << 11 |
            (shift & mask(2)) << 12;
  ...
  if (fCode) {
    ...
    if (l->kind == Label::ARMDisp19) {
      for (int ref : l->references) {
        // ref points to a 32-bit instruction with 19-bit displacement in
        // instructions.
        uint32_t inst;
        memcpy(&inst, fCode + ref, 4);

        // [ 8 bits to preserve] [ 19 bit signed displacement ] [ 5 bits to
        // preserve ]
...
#define SKVM_JIT_STATS 0
  ...
  // jit_entry may be null either simply because we can't JIT, or when using
  // LLVM if the work represented by fImpl->llvm_compiling hasn't finished yet.
  //
  // Ordinarily we'd never find ourselves with non-null jit_entry and
  // !gSkVMAllowJIT, but it can happen during interactive programs like Viewer
  // that toggle gSkVMAllowJIT on and off, due to timing or program caching.
  if (jit_entry != nullptr && gSkVMAllowJIT) {
    ...
    switch (fImpl->strides.size()) {
      ...
      case 3:
        return ((void (*)(int, void*, void*, void*))jit_entry)(n, a[0], a[1],
                                                               a[2]);
      case 4:
        return ((void (*)(int, void*, void*, void*, void*))jit_entry)(
            n, a[0], a[1], a[2], a[3]);
      case 5:
      ...
      case 6:
        return (
            (void (*)(int, void*, void*, void*, void*, void*, void*))jit_entry)(
            n, a[0], a[1], a[2], a[3], a[4], a[5]);
      case 7:
        return ((void (*)(int, void*, void*, void*, void*, void*, void*,
                          void*))jit_entry)(n, a[0], a[1], a[2], a[3], a[4],
                                            a[5], a[6]);
      default:
  ...
  // So we'll sometimes use the interpreter here even if later calls will use
  // the JIT.
  SkOpts::interpret_skvm(fImpl->instructions.data(),
                         (int)fImpl->instructions.size(), this->nregs(),
                         this->loop(), fImpl->strides.data(),
...
void Program::setupLLVM(const std::vector<OptimizedInstruction>& instructions,
  ...
  llvm::FunctionType* fn_type = llvm::FunctionType::get(
      llvm::Type::getVoidTy(*ctx), arg_types, /*vararg?=*/false);
  llvm::Function* fn = llvm::Function::Create(
      fn_type, llvm::GlobalValue::ExternalLinkage, debug_name, *mod);
  for (size_t i = 0; i < fImpl->strides.size(); i++) {
  ...
  auto emit = [&](size_t i, bool scalar, IRBuilder* b) {
    ...
    auto S = [&](llvm::Type* dst, llvm::Value* v) {
      return b->CreateSExt(v, dst);
    };
    ...
    switch (llvm::Type* t = nullptr; op) {
      ...
      case Op::splat:
        vals[i] = llvm::ConstantInt::get(I32, immA);
        break;
      ...
      store : {
        ...
        llvm::Value* ptr =
            b->CreateBitCast(args[immA], val->getType()->getPointerTo());
      ...
      case Op::select:
        vals[i] =
            b->CreateSelect(b->CreateTrunc(vals[x], I1), vals[y], vals[z]);
        break;
      ...
      case Op::shl_i32:
        vals[i] = b->CreateShl(vals[x], immA);
        break;
      case Op::sra_i32:
        vals[i] = b->CreateAShr(vals[x], immA);
        break;
      case Op::shr_i32:
        vals[i] = b->CreateLShr(vals[x], immA);
        break;
      ...
      case Op::fms_f32:
        vals[i] = I(b->CreateIntrinsic(
            llvm::Intrinsic::fma, {F32},
            {F(vals[x]), F(vals[y]), b->CreateFNeg(F(vals[z]))}));
        break;

      case Op::fnma_f32:
        vals[i] = I(b->CreateIntrinsic(
            llvm::Intrinsic::fma, {F32},
            {b->CreateFNeg(F(vals[x])), F(vals[y]), F(vals[z])}));
        break;
      ...
      case Op::floor:
        vals[i] =
            I(b->CreateUnaryIntrinsic(llvm::Intrinsic::floor, F(vals[x])));
        break;
      ...
      case Op::round: {
        ...
        if (scalar) {
          ...
          llvm::Value* v =
              llvm::UndefValue::get(llvm::VectorType::get(f32, 4, false));
          v = b->CreateInsertElement(v, F(vals[x]), (uint64_t)0);
          vals[i] =
              b->CreateIntrinsic(llvm::Intrinsic::x86_sse_cvtss2si, {}, {v});
        } else {
          SkASSERT(K == 4 || K == 8);
          auto intr =
              K == 4 ? llvm::Intrinsic::x86_sse2_cvtps2dq :
  ...
  // and pointer aliasing makes it easier to manually hoist than teach LLVM it's
  // safe.
  {
    IRBuilder b(hoistK);

    // Hoisted instructions will need args (think, uniforms), so set that up
    // now. These phi nodes are degenerate... they'll always be the passed-in
    // args from enter. Later on when we start looping the phi nodes will start
    // looking useful.
    llvm::Argument* arg = fn->arg_begin();
    (void)arg++;  // Leave n as nullptr... it'd be a bug to use n in a hoisted
                  // instruction.
    for (size_t i = 0; i < fImpl->strides.size(); i++) {
  ...
  // testK:  if (N >= K) goto loopK; else goto hoist1;
  {
    ...
    // New phi nodes for `n` and each pointer argument from hoistK; later we'll
    // add loopK. These also start as the initial function arguments; hoistK
  ...
  // loopK:  ... insts on K x T vectors; N -= K, args += K*stride; goto testK;
  {
    ...
    // Each arg ptr += K
    for (size_t i = 0; i < fImpl->strides.size(); i++) {
      llvm::Value* arg_next = b.CreateConstInBoundsGEP1_32(
          llvm::Type::getInt8Ty(*ctx), args[i], K * fImpl->strides[i]);
  ...
  // test1:  if (N >= 1) goto loop1; else goto leave;
  {
    ...
    // Set up new phi nodes for `n` and each pointer argument, now from hoist1
    // and loop1.
...
// We have to be careful here about what we close over and how, in case fImpl
// moves. fImpl itself may change, but its pointee fields won't, so close over
// them by value. Also, debug_name will almost certainly leave scope, so copy
// it.
#else
    fImpl->llvm_compiling = std::async(
        std::launch::async, [dst = &fImpl->jit_entry, ee = fImpl->llvm_ee.get(),
...
Program::Program(const std::vector<OptimizedInstruction>& instructions,
  ...
  // Might as well do this after setupLLVM() to get a little more time to
  // compile.
...
void Program::setupInterpreter(
  ...
  auto assign_register = [&](Val id) {
    ...
    if (inst.death != id) {
      // Allocate a register if we have to, preferring to reuse anything
      // available.
      if (avail.empty()) {
        reg[id] = fImpl->regs++;
      } else {
  ...
  // Assign a register to each hoisted instruction, then each non-hoisted loop
  // instruction.
  ...
  // Translate OptimizedInstructions to InterpreterIstructions by mapping values
  // to registers.  This will be two passes, first hoisted instructions, then
  // inside the loop.
...
bool Program::jit(const std::vector<OptimizedInstruction>& instructions,
  ...
  SkTHashMap<int, A::Label>
      constants;          // Constants (mostly splats) share the same pool.
  ...
  auto exit = [&] {
    // The second pass of jit() shouldn't use any register it didn't in the
    // first pass.
  ...
  // We can use v0-v7 and v16-v31 freely; we'd need to preserve v8-v15 in
  // enter/exit.
  ...
  *registers_used = 0;  // We'll update this as we go.
  ...
  auto emit = [&](Val id, bool scalar) {
    ...
    // each freed manually with free_tmp() or noted as our result with
    // mark_tmp_as_dst().
    auto alloc_tmp = [&](int N = 1) -> Reg {
      auto needs_spill = [&](Val v) -> bool {
        SkASSERT(v >=
                 0);  // {NA,TMP,RES} need to be handled before calling this.
        return stack_slot[v] == NA  // We haven't spilled it already?
               &&
               instructions[v].op != Op::splat;  // No need to spill constants.
      };

      // We want to find a block of N adjacent registers requiring the fewest
      // spills.
      ...
      for (int block = 0; block + N <= (int)regs.size(); block++) {
        ...
        for (int r = block; r < block + N; r++) {
          ...
          // We can't spill anything REServed or that we'll need this
          // instruction.
          ...
          // Usually here we've got a value v that we'd have to spill to the
          // stack before reusing its register, but sometimes even now we get a
          // freebie.
      ...
      // TODO: our search's success isn't obviously guaranteed... it depends on
      // N and the number and relative position in regs of any unspillable
      // values. I think we should be able to get away with N≤2 on x86-64 and
      // N≤4 on arm64; we'll need to revisit this logic should this assert fire.
    ...
    // Which register holds dst,x,y,z,w for this instruction?  NA if none does
    // yet.
    ...
    auto try_alias = [&](Val v) -> bool {
      ...
      if (dies_here(v)) {
        ...
        regs[rd] =
            id;  // Next instruction, Val id will be in the register, not Val v.
        return true;
      }
      return false;
    };
...
#if defined(__x86_64__) || defined(_M_X64)
    // On x86 we can work with many values directly from the stack or program
    // constant pool.
    ...
    // if we need this value in ARM, might as well just call r(v) to get it into
    // a register.
    ...
    switch (op) {
      ...
      case Op::gather8: {
        ...
        for (int i = 0; i < active_lanes; i++) {
          ...
          a->vpinsrb((A::Xmm)dst(), (A::Xmm)dst(), A::Mem{GP0, 0, GP1, A::ONE},
                     i);
        }
        a->vpmovzxbd(dst(), dst());
        free_tmp(tmp);
      ...
      case Op::gather16: {
        // Just as gather8 except vpinsrb->vpinsrw, ONE->TWO, and
        // vpmovzxbd->vpmovzxwd.
        a->mov(GP0, A::Mem{arg[immA], immB});
        ...
        for (int i = 0; i < active_lanes; i++) {
          ...
          a->vpextrd(GP1, (A::Xmm)tmp, i % 4);
          a->vpinsrw((A::Xmm)dst(), (A::Xmm)dst(), A::Mem{GP0, 0, GP1, A::TWO},
                     i);
        }
        a->vpmovzxwd(dst(), dst());
        free_tmp(tmp);
      ...
      // We can swap the arguments of symmetric instructions to make better use
      // of any().
      case Op::add_f32:
        if (in_reg(x)) {
      ...
      case Op::store128:
        if (scalar) {
          ...
          Reg tmp0 = alloc_tmp(4), tmp1 = (Reg)(tmp0 + 1),
              tmp2 = (Reg)(tmp0 + 2), tmp3 = (Reg)(tmp0 + 3);
          a->orr16b(tmp0, r(x), r(x));
          a->orr16b(tmp1, r(y), r(y));
      ...
      // See gather8 for general idea; comments here only where gather16
      // differs.
      case Op::gather16: {
        a->add(GP0, arg[immA], immB);
        a->ldrd(GP0, GP0);
        for (int i = 0; i < active_lanes; i++) {
          a->movs(GP1, r(x), i);
          a->add(GP1, GP0, GP1, A::LSL,
                 1);          // Scale index 2x into a byte offset.
          a->ldrh(GP1, GP1);  // 2-byte load.
          a->inss(dst(x), GP1, i);
        }
      } break;

      // See gather8 for general idea; comments here only where gather32
      // differs.
      case Op::gather32: {
        a->add(GP0, arg[immA], immB);
        a->ldrd(GP0, GP0);
        for (int i = 0; i < active_lanes; i++) {
          a->movs(GP1, r(x), i);
          a->add(GP1, GP0, GP1, A::LSL,
                 2);          // Scale index 4x into a byte offset.
      ...
      case Op::from_fp16:
        a->xtns2h(dst(x),
                  r(x));  // pack even 16-bit lanes into bottom four lanes
  ...
  // This point marks a kind of canonical fixed point for register contents: if
  // loop code is generated as if these registers are holding these values, the
  // next time the loop comes around we'd better find those same registers
  // holding those same values.
  auto restore_incoming_regs = [&, incoming = regs,
                                saved_stack_slot = stack_slot,
...
void Program::setupJIT(const std::vector<OptimizedInstruction>& instructions,
                       const char* debug_name) {
  // Assemble with no buffer to determine a.size() (the number of bytes we'll
  // assemble) and stack_hint/registers_used to feed forward into the next jit()
  // call.
  Assembler a{nullptr};
  int stack_hint = -1;
  ...
  // Assemble the program for real with stack_hint/registers_used as feedback
  // from first call.
...
void Program::disassemble(SkWStream* o) const {
  ...
  // TODO: We could trim this down to just our code using '--disassemble=<symbol
  // name>`, but the symbol name varies with OS, and that option may be missing
  // from objdump on some machines? There also apears to be quite a bit of junk
  // after the end of the JIT'd code. Trimming that would let us pass
  // '--visualize-jumps' and get the loop annotated. With the junk, we tend to
  // end up with a bunch of stray jumps that pollute the ASCII art.
