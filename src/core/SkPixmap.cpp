...
void SkPixmap::reset(const SkImageInfo& info, const void* addr,
                     size_t rowBytes) {
  if (addr) {
...
bool SkPixmap::extractSubset(SkPixmap* result, const SkIRect& subset) const {
  ...
  // If the upper left of the rectangle was outside the bounds of this SkBitmap,
  // we should have exited above.
  SkASSERT(static_cast<unsigned>(r.fLeft) <
           static_cast<unsigned>(this->width()));
  SkASSERT(static_cast<unsigned>(r.fTop) <
...
// This is the same as SkPixmap::addr(x,y), but this version gets inlined, while
// the public method does not. Perhaps we could bloat it so it can be inlined,
// but that would grow code-size everywhere, instead of just here (on behalf of
// getAlphaf()).
...
bool SkPixmap::readPixels(const SkImageInfo& dstInfo, void* dstPixels,
                          size_t dstRB, int x, int y) const {
  ...
  return SkConvertPixels(rec.fInfo, rec.fPixels, rec.fRowBytes, srcInfo,
                         srcPixels, this->rowBytes());
}
...
bool SkPixmap::erase(const SkColor4f& color, SkColorSpace* cs,
...
bool SkPixmap::scalePixels(const SkPixmap& actualDst,
  ...
  // We may need to tweak how we interpret these just a little below, so we make
  // copies.
  ...
  // If src and dst are both unpremul, we'll fake the source out to appear as if
  // premul, and mark the destination as opaque.  This odd combination allows us
  // to scale unpremul pixels without ever premultiplying them (perhaps losing
  // information in the color channels). This is an idiosyncratic feature of
  // scalePixels(), and is tested by scalepixels_unpremul GM.
  bool clampAsIfUnpremul = false;
  if (src.alphaType() == kUnpremul_SkAlphaType &&
      dst.alphaType() == kUnpremul_SkAlphaType) {
    src.reset(src.info().makeAlphaType(kPremul_SkAlphaType), src.addr(),
              src.rowBytes());
    dst.reset(dst.info().makeAlphaType(kOpaque_SkAlphaType), dst.addr(),
              dst.rowBytes());
  ...
  SkMatrix scale = SkMatrix::RectToRect(SkRect::Make(src.bounds()),
                                        SkRect::Make(dst.bounds()));

  sk_sp<SkShader> shader = SkImageShader::Make(
      bitmap.asImage(), SkTileMode::kClamp, SkTileMode::kClamp, sampling,
      &scale, clampAsIfUnpremul);

  sk_sp<SkSurface> surface = SkSurface::MakeRasterDirect(
      dst.info(), dst.writable_addr(), dst.rowBytes());
  if (!shader || !surface) {
    return false;
  }
...
static bool draw_orientation(const SkPixmap& dst, const SkPixmap& src,
                             SkEncodedOrigin origin) {
  auto surf = SkSurface::MakeRasterDirect(dst.info(), dst.writable_addr(),
                                          dst.rowBytes());
  if (!surf) {
    return false;
  }
  ...
  surf->getCanvas()->drawImage(SkImage::MakeFromBitmap(bm), 0, 0,
                               SkSamplingOptions(), &p);
  return true;
}

bool SkPixmapPriv::Orient(const SkPixmap& dst, const SkPixmap& src,
                          SkEncodedOrigin origin) {
