...
#endif
  return SkM44((1.f / 6) * B, -(3.f / 6) * B - C, (3.f / 6) * B + 2 * C,
               -(1.f / 6) * B - C, 1 - (2.f / 6) * B, 0,
               -3 + (12.f / 6) * B + C, 2 - (9.f / 6) * B - C, (1.f / 6) * B,
               (3.f / 6) * B + C, 3 - (15.f / 6) * B - 2 * C,
               -2 + (9.f / 6) * B + C, 0, 0, -C, (1.f / 6) * B + C);
}
...
static SkTileMode optimize(SkTileMode tm, int dimension) {
  ...
  // need to update
  // frameworks/base/libs/hwui/tests/unit/SkiaBehaviorTests.cpp:55 to allow for
  // transforming to clamp.
  return tm;
#else
  // mirror and repeat on a 1px axis are the same as clamping, but decal will
  // still transition to transparent black.
  return (tm != SkTileMode::kDecal && dimension == 1) ? SkTileMode::kClamp : tm;
#endif
}

// TODO: currently this only *always* used in asFragmentProcessor(), which is
// excluded on no-gpu builds. No-gpu builds only use needs_subset() in asserts,
// so release+no-gpu doesn't use it, which can cause builds to fail if unused
// warnings are treated as errors.
SK_MAYBE_UNUSED static bool needs_subset(SkImage* img, const SkRect& subset) {
  return subset != SkRect::Make(img->dimensions());
}

SkImageShader::SkImageShader(sk_sp<SkImage> img, const SkRect& subset,
                             SkTileMode tmx, SkTileMode tmy,
                             const SkSamplingOptions& sampling,
                             const SkMatrix* localMatrix, bool raw,
  ...
  // Bicubic filtering of raw image shaders would add a surprising clamp - so we
  // don't support it
...
sk_sp<SkFlattenable> SkImageShader::CreateProc(SkReadBuffer& buffer) {
  ...
  bool raw = buffer.isVersionLT(SkPicturePriv::Version::kRawImageShaders)
                 ? false
                 : buffer.readBool();

  // TODO(skbug.com/12784): Subset is not serialized yet; it's only used by
  // special images so it will never be written to an SKP.

  return raw ? SkImageShader::MakeRaw(std::move(img), tmx, tmy, sampling,
                                      &localMatrix)
             : SkImageShader::Make(std::move(img), tmx, tmy, sampling,
                                   &localMatrix);
}
...
sk_sp<SkShader> SkImageShader::Make(sk_sp<SkImage> image, SkTileMode tmx,
                                    SkTileMode tmy,
                                    const SkSamplingOptions& options,
                                    const SkMatrix* localMatrix,
                                    bool clampAsIfUnpremul) {
  SkRect subset =
      image ? SkRect::Make(image->dimensions()) : SkRect::MakeEmpty();
  return MakeSubset(std::move(image), subset, tmx, tmy, options, localMatrix,
                    clampAsIfUnpremul);
}

sk_sp<SkShader> SkImageShader::MakeRaw(sk_sp<SkImage> image, SkTileMode tmx,
                                       SkTileMode tmy,
                                       const SkSamplingOptions& options,
                                       const SkMatrix* localMatrix) {
  if (options.useCubic) {
...
sk_sp<SkShader> SkImageShader::MakeSubset(sk_sp<SkImage> image,
                                          const SkRect& subset, SkTileMode tmx,
                                          SkTileMode tmy,
                                          const SkSamplingOptions& options,
                                          const SkMatrix* localMatrix,
                                          bool clampAsIfUnpremul) {
  ...
  // TODO(skbug.com/12784): GPU-only for now since it's only supported in
  // onAsFragmentProcessor()
  SkASSERT(!needs_subset(image.get(), subset) || image->isTextureBacked());
  return sk_sp<SkShader>{new SkImageShader(image, subset, tmx, tmy, options,
                                           localMatrix, /*raw=*/false,
                                           clampAsIfUnpremul)};
}
...
std::unique_ptr<GrFragmentProcessor> SkImageShader::asFragmentProcessor(
  ...
  const SkRect* subset =
      needs_subset(fImage.get(), fSubset) ? &fSubset : nullptr;
  auto fp = as_IB(fImage.get())
                ->asFragmentProcessor(args.fContext, fSampling, tileModes,
                                      lmInverse, subset);
  if (!fp) {
    return nullptr;
  }
  ...
  if (!fRaw) {
    fp = GrColorSpaceXformEffect::Make(
        std::move(fp), fImage->colorSpace(), fImage->alphaType(),
...
void SkImageShader::addToKey(const SkKeyContext& keyContext,
  ...
  ImageShaderBlock::ImageData imgData(fSampling, fTileModeX, fTileModeY,
                                      fSubset, this->getLocalMatrix());

#ifdef SK_GRAPHITE_ENABLED
  if (as_IB(fImage)->isGraphiteBacked()) {
    skgpu::graphite::Image* grImage =
        static_cast<skgpu::graphite::Image*>(fImage.get());
...
sk_sp<SkShader> SkMakeBitmapShaderForPaint(const SkPaint& paint,
                                           const SkBitmap& src, SkTileMode tmx,
                                           SkTileMode tmy,
                                           const SkSamplingOptions& sampling,
                                           const SkMatrix* localMatrix,
                                           SkCopyPixelsMode mode) {
  auto s = SkImageShader::Make(SkMakeImageFromRasterBitmap(src, mode), tmx, tmy,
                               sampling, localMatrix);
  if (!s) {
    return nullptr;
  }
  if (SkColorTypeIsAlphaOnly(src.colorType()) && paint.getShader()) {
    // Compose the image shader with the paint's shader. Alpha images+shaders
    // should output the texture's alpha multiplied by the shader's color. DstIn
    // (d*sa) will achieve this with the source image and dst shader (MakeBlend
    // takes dst first, src second).
...
class SkImageShader::TransformShader : public SkTransformShader {
 public:
  ...
  skvm::Color onProgram(skvm::Builder* b, skvm::Coord device, skvm::Coord local,
                        skvm::Color color, const SkMatrixProvider& matrices,
                        const SkMatrix* localM, const SkColorInfo& dst,
                        skvm::Uniforms* uniforms,
                        SkArenaAlloc* alloc) const override {
    return fImageShader.makeProgram(b, device, local, color, matrices, localM,
                                    dst, uniforms, this, alloc);
  }
...
bool SkImageShader::doStages(const SkStageRec& rec,
                             TransformShader* updater) const {
  SkASSERT(!needs_subset(fImage.get(), fSubset));  // TODO(skbug.com/12784)
  ...
  auto* access =
      SkMipmapAccessor::Make(alloc, fImage.get(), matrix, sampling.mipmap);
  if (!access) {
    return false;
  }
  SkPixmap pm;
  ...
  if (sampling.useCubic) {
    CubicResamplerMatrix(sampling.cubic.B, sampling.cubic.C)
  ...
  auto append_misc = [&] {
    ...
    // Transform color space and alpha type to match shader convention (dst CS,
    // premul alpha).
    if (!fRaw) {
      alloc
          ->make<SkColorSpaceXformSteps>(cs, at, rec.fDstCS,
                                         kPremul_SkAlphaType)
          ->apply(p);
    }
  ...
  if (true &&
      (ct == kRGBA_8888_SkColorType ||
       ct == kBGRA_8888_SkColorType)  // TODO: all formats
      && !sampling.useCubic && sampling.filter == SkFilterMode::kLinear &&
  ...
  if (true && (ct == kRGBA_8888_SkColorType || ct == kBGRA_8888_SkColorType) &&
      sampling.useCubic && fTileModeX == SkTileMode::kClamp &&
  ...
  SkRasterPipeline_SamplerCtx* sampler =
      alloc->make<SkRasterPipeline_SamplerCtx>();
...
SkStageUpdater* SkImageShader::onAppendUpdatableStages(
    const SkStageRec& rec) const {
...
skvm::Color SkImageShader::onProgram(skvm::Builder* b, skvm::Coord device,
                                     skvm::Coord origLocal, skvm::Color paint,
                                     const SkMatrixProvider& matrices,
                                     const SkMatrix* localM,
                                     const SkColorInfo& dst,
                                     skvm::Uniforms* uniforms,
                                     SkArenaAlloc* alloc) const {
  return this->makeProgram(b, device, origLocal, paint, matrices, localM, dst,
                           uniforms, nullptr, alloc);
}

skvm::Color SkImageShader::makeProgram(
    skvm::Builder* p, skvm::Coord device, skvm::Coord origLocal,
    skvm::Color paint, const SkMatrixProvider& matrices, const SkMatrix* localM,
    const SkColorInfo& dst, skvm::Uniforms* uniforms,
    const TransformShader* coordShader, SkArenaAlloc* alloc) const {
  SkASSERT(!needs_subset(fImage.get(), fSubset));  // TODO(skbug.com/12784)
  SkMatrix baseInv;
  ...
  auto* access =
      SkMipmapAccessor::Make(alloc, fImage.get(), baseInv, sampling.mipmap);
  if (!access) {
    return {};
  }

  auto [upper, upperInv] = access->level();
  // If we are using a coordShader, then we can't make guesses about the state
  // of the matrix.
  ...
  // more than once. To avoid adding/registering them multiple times, we
  // pre-load them into a struct (just to logically group them together), based
  // on the "current" pixmap (level of a mipmap).
  //
  struct Uniforms {
    ...
    skvm::PixelFormat
        pixelFormat;  // not a uniform, but needed for each texel sample,
  ...
  auto setup_uniforms = [&](const SkPixmap& pm) -> Uniforms {
    skvm::PixelFormat pixelFormat =
        skvm::SkColorType_to_PixelFormat(pm.colorType());
    return {
        p->uniformF(uniforms->pushF(pm.width())),
        p->uniformF(
            uniforms->pushF(1.0f / pm.width())),  // iff tileX == kRepeat
        p->uniformF(
            uniforms->pushF(0.5f / pm.width())),  // iff tileX == kMirror

        p->uniformF(uniforms->pushF(pm.height())),
        p->uniformF(
            uniforms->pushF(1.0f / pm.height())),  // iff tileY == kRepeat
        p->uniformF(
            uniforms->pushF(0.5f / pm.height())),  // iff tileY == kMirror
  ...
  auto sample_texel = [&](const Uniforms& u, skvm::F32 sx,
                          skvm::F32 sy) -> skvm::Color {
    // repeat() and mirror() are written assuming they'll be followed by a
    // [0,scale) clamp.
    ...
    // safety and to handle the clamps still needed by kClamp, kRepeat, and
    // kMirror.
    ...
    // If we know the image is opaque, jump right to alpha = 1.0f, skipping work
    // to unpack it.
    if (input_is_opaque) {
      c.a = p->splat(1.0f);
    }

    // Mask away any pixels that we tried to sample outside the bounds in
    // kDecal.
    if (fTileModeX == SkTileMode::kDecal || fTileModeY == SkTileMode::kDecal) {
  ...
  auto sample_level = [&](const SkPixmap& pm, const SkMatrix& inv,
                          skvm::Coord local) {
    const Uniforms u = setup_uniforms(pm);

    if (sampling.useCubic) {
      // All bicubic samples have the same fractional offset (fx,fy) from the
      // center. They're either the 16 corners of a 3x3 grid/ surrounding (x,y)
      // at (0.5,0.5) off-center.
      ...
      skvm::F32 left = local.x - 0.5f, top = local.y - 0.5f,
                right = local.x + 0.5f, bottom = local.y + 0.5f;

      // The fractional parts of right and bottom are our lerp factors in x and
      // y respectively.
      skvm::F32 fx = fract(right), fy = fract(bottom);

      return lerp(
          lerp(sample_texel(u, left, top), sample_texel(u, right, top), fx),
          lerp(sample_texel(u, left, bottom), sample_texel(u, right, bottom),
               fx),
          fy);
    } else {
  ...
  if (lower) {
    auto lowerLocal =
        SkShaderBase::ApplyMatrix(p, lowerInv, origLocal, uniforms);
    // lower * weight + upper * (1 - weight)
    c = lerp(c, sample_level(*lower, lowerInv, lowerLocal),
             p->uniformF(uniforms->pushF(lowerWeight)));
  }

  // If the input is opaque and we're not in decal mode, that means the output
  // is too. Forcing *a to 1.0 here will retroactively skip any work we did to
  // interpolate sample alphas.
  if (input_is_opaque && fTileModeX != SkTileMode::kDecal &&
      fTileModeY != SkTileMode::kDecal) {
    c.a = p->splat(1.0f);
  }

  // Alpha-only images get their color from the paint (already converted to dst
  // color space).
  ...
  return fRaw
             ? c
             : SkColorSpaceXformSteps{cs, at, dst.colorSpace(), dst.alphaType()}
